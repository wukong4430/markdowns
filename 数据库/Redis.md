...
Redis通过**AE事件模型**以及**IO多路复用**等技术，处理性能非常高，因此没有必要使用多线程。



在redis6.0之后开放了多线程，这里的多线程是用来处理其他的一些任务的，并不是使用多线程来执行redis的读写操作。

其实在redis4.0之后，已经不是严格的单线程了。除了一个主线程（读、解析、执行、写），它也有后台线程在处理一些较为缓慢的操作，例如清理脏数据、无用连接的释放、大 key 的删除等等。

redis6.0之前不使用多线程的原因是CPU几乎不可能成为瓶颈。主要是受限于网络和内存。	使用单线程，可维护性高。不会带来并发操作的各种线程安全问题。无论是从实际编码和线程维护来考虑，单线程都要方便的多。



而Redis6.0以后支持多线程主要是两个原因：
