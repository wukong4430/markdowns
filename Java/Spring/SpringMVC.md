...
AspectJ是编译时的代理模式体现，在编译阶段就将切面织入字节码文件当中。

Spring AOP是动态代理的实现，不是在编译阶段将增强的功能编制到字节码当中，而是在程序运行的过程中，通过反射机制动态地将额外的功能添加到原本的代码中去。
...
观察者模式是一个一对多的关系。有一个主题，然后有很多的观察者关注着这个主题，一旦这个主题上的内容发生了变化。这个主题会立刻提醒所有的观察者进行数据的更新。
